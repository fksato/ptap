...
class AssetDiskIO{
    ...
    async load_asset(assetPath, assetType){
        return
    }

    async load_video(blobpath) {
        return
    }
...
class S3_IO{
    constructor(){
        ...
        this.load_asset = this._load_asset.bind(this)
        this.load_image = this._load_image.bind(this)
        this.load_video = this._load_video.bind(this)
        this.load_sound = this._load_sound.bind(this)
        this.exists = this._exists.bind(this)

    }

    async _load_asset(assetPath, assetType) {
        if (assetType=='image') {
            return await this._load_image(assetPath)
        }
        else if (assetType == 'video') {
            return await this._load_video(assetPath)
        }        
    }

    async _load_video(vid_url) {
        var resolvefunc 
        var rejectfunc 
        ...
        xhttp.open("GET", vid_url, true);
        xhttp.send();
        var vid_blob = await p
        ...
        var p = new Promise(
            function(resolve, reject){
                try{
                    ...
                    var retry_number = 0; 
                    try{
                        _this.dbx.filesDownload({path: vidpath}).then( 
                            function(data){
